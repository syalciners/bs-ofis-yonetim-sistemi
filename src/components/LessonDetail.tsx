...
export function LessonDetail({ lesson, onEdit, onDone }: {lesson:Ders;onEdit:()=>void;onDone:()=>void}) {
  ...
  if(!data)return null
  const setStatus=async(status:string)=>{if(status===lesson.ders_durumu)return;setBusy(true);try{await setLessonStatus(lesson.ders_id,status);await refresh();toast(`Ders durumu “${status}” olarak kaydedildi.`);onDone()}catch(e:any){toast(e.message||String(e),'error')}finally{setBusy(false)}}
  ...
  return <div className="detail-stack lesson-detail-v2">
    ...
    <section className="lesson-result-section lesson-result-simple">
      <div className="section-heading compact"><div><h3>Ders Durumu</h3><span>Tek dokunuşla değiştir.</span></div></div>

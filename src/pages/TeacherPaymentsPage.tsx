...
export function TeacherPaymentsPage(){
  ...
  const rows=teachers.map(tch=>{
    const lessons=period?data.dersler.filter(x=>x.ogretmen_id===tch.ogretmen_id&&x.ders_durumu==='Yapıldı'&&(x.tarih||'')>=period.baslangic_tarihi&&(x.tarih||'')<=period.bitis_tarihi):[]
    const earned=lessons.reduce((s,x)=>s+Number(x.ogretmen_toplam_hakedis||0),0)
    const paid=period?data.ogretmenOdemeleri.filter(x=>x.ogretmen_id===tch.ogretmen_id&&x.hakedis_donemi_id===period.hakedis_donemi_id&&!x.iptal_mi).reduce((s,x)=>s+Number(x.tutar||0),0):0
    return {teacher:tch,earned,paid,remaining:Math.max(earned-paid,0),lessonHours:lessons.reduce((sum,x)=>sum+Number(x.ders_sayisi||1),0),branches:branchNames(tch.ogretmen_id)}
  ...
  const cancelSelected=async()=>{if(!selected||selected.iptal_mi)return;if(!window.confirm(`${t.teacher} ödemesi iptal edilsin mi?\n\nKayıt silinmeyecek; bağlı kasa hareketi de aynı işlemde iptal edilecek.`))return;setCancelBusy(true);try{await cancelTeacherPayment(selected.ogretmen_odeme_id,'Kullanıcı tarafından iptal edildi');await refresh();toast(`${t.teacher} ödemesi iptal edildi.`);setSelected(null)}catch(e:any){toast(e.message||String(e),'error')}finally{setCancelBusy(false)}}

  return <div className="page-stack teacher-payments-page">
    <section className="page-title-row"><div><span className="eyebrow">{t.teacher.toLocaleUpperCase('tr-TR')} YÖNETİMİ</span><h1>{t.teacher} Ödemeleri</h1></div><button className="primary-btn" onClick={()=>setPaymentTeacher('')}><Plus size={17}/>Ödeme Yap</button></section>
    ...
    <section className="teacher-payment-toolbar">
      ...
      <label><span>{t.teacher}</span><select value={teacherFilter} onChange={e=>setTeacherFilter(e.target.value)}><option value="tum">Tüm {t.teachers}</option>{data.ogretmenler.filter(x=>x.durum!=='Pasif').map(x=><option key={x.ogretmen_id} value={x.ogretmen_id}>{x.ad_soyad}</option>)}</select></label>
    </section>
    ...
    <section className="kpi-grid four compact-kpis teacher-payment-kpis">
      ...
      <div className="kpi-card red"><span>Ödeme Bekleyen</span><strong>{dueCount}</strong><small>{t.teacherLower}</small></div>
    </section>

    <div className="section-heading"><div><h2>{t.teacher} Hakedişleri</h2><span>{period?.donem_adi||'Dönem seçin'}</span></div></div>
    <section className="teacher-payment-grid">{rows.map(x=><button key={x.teacher.ogretmen_id} className={`teacher-payment-card ${teacherTone(x.teacher.ad_soyad)} ${x.remaining<=0?'paid-up':''}`} onClick={()=>setPaymentTeacher(x.teacher.ogretmen_id)}>
      <div className="teacher-payment-card-head"><div className="avatar purple">{x.teacher.ad_soyad.split(/\s+/).slice(0,2).map(y=>y[0]).join('').toLocaleUpperCase('tr-TR')}</div><div><strong>{x.teacher.ad_soyad}</strong><span>{x.branches.length?x.branches.join(' · '):`${t.branch} tanımlanmamış`}</span></div>{x.remaining>0?<span className="soft-pill danger-soft">Ödeme Bekliyor</span>:<span className="soft-pill success-soft">Tamam</span>}</div>
    ...
    {!rows.length&&<div className="calm-empty"><GraduationCap/><b>Bu filtrede {t.teacherLower} bulunamadı.</b><span>{t.teacher} veya hakediş dönemi filtresini değiştirin.</span></div>}

...
export function LessonCard({ lesson, onClick, compact=false }: { lesson:Ders;onClick:()=>void;compact?:boolean }) {
  ...
  return <button type="button" className={`lesson-card status-${status} ${compact?'compact':''}`} onClick={onClick}>
    <div className="lesson-time"><strong>{time(lesson.baslangic_saati)}</strong><span>{Number(lesson.ders_sayisi||1)} ders</span></div>
    <div className="lesson-main"><strong>{studentName(data,lesson.ogrenci_id)}</strong><small>{teacherName(data,lesson.ogretmen_id)} · {branchName(data,lesson.brans_id)} · {roomName(data,lesson.derslik_id)}</small></div>

...
export function FinancePage(){
  const {data,refresh}=useAppData();const{toast}=useToast();const nav=useNavigate();const[params]=useSearchParams();const init=(params.get('tab') as Tab)||'tahsilatlar';const[tab,setTab]=useState<Tab>(init);const[add,setAdd]=useState<Tab|null>(null);const[paymentTeacher,setPaymentTeacher]=useState<string|null>(null);const[selected,setSelected]=useState<any|null>(null);const[cancelBusy,setCancelBusy]=useState(false);const[showCanceledCollections,setShowCanceledCollections]=useState(false)
  ...
  return <div className="page-stack">
    ...
    {tab==='tahsilatlar'&&<>
      <div className="section-heading"><div><h2>Son Tahsilatlar</h2><span>{activeCollectionCount} aktif kayıt</span></div><div>{canceledCollectionCount>0&&<button type="button" className="text-btn" aria-pressed={showCanceledCollections} onClick={()=>setShowCanceledCollections(x=>!x)}>{showCanceledCollections?'İptalleri Gizle':`İptalleri Göster (${canceledCollectionCount})`}</button>}<button type="button" className="text-btn" onClick={()=>nav('/ogrenciler?filtre=borclu')}>Borçlu Öğrenciler</button></div></div>
      <section className="finance-list">{visibleCollections.length?visibleCollections.map(x=><button className="finance-card income" style={x.iptal_mi?{opacity:.5}:undefined} key={x.tahsilat_id} onClick={()=>setSelected({type:'tahsilat',row:x})}><div className="finance-icon"><Banknote/></div><div><strong>{studentName(data,x.ogrenci_id)}</strong><small>{fullDate(x.tarih)} · {x.odeme_yontemi||'—'}{x.iptal_mi?' · İptal':''}{x.aciklama?` · ${x.aciklama}`:''}</small></div><b>{x.iptal_mi?'İptal · ':''}{money(x.tutar)}</b></button>):<div className="calm-empty"><Users/><b>Aktif tahsilat yok.</b><span>{canceledCollectionCount?'İptal edilen kayıtlar varsayılan listede gizleniyor.':'İlk tahsilatı “Tahsilat Al” ile kaydedebilirsin.'}</span></div>}</section>
    </>}

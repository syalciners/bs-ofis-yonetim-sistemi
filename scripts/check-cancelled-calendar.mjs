...
if(!weekly.includes('CALENDAR_HIDDEN_STATUSES') || !weekly.includes('pdfLessons.filter(x=>!CALENDAR_HIDDEN_STATUSES.has(String(x.ders_durumu||\'\')))')) failures.push('Haftalık Takvim iptal/ertelenen dersleri görünümden çıkarmıyor.')
...
if(!weekly.includes('openWeeklyProgramPdf(data,pdfLessons')) failures.push('PDF, iptal derslerden arındırılmış Takvim listesini kullanıyor.')
if(!weekly.includes('disabled={!pdfLessons.length}')) failures.push('Yalnız iptal ders olan haftada PDF butonu kullanılamıyor.')

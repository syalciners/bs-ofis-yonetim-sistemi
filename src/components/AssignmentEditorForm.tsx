...
export function AssignmentEditorForm({ assignment, studentId, onDone, onCancel }: { assignment?:Odev; studentId?:string; onDone:()=>void; onCancel:()=>void }) {
  ...
  const submit=async(e:React.FormEvent<HTMLFormElement>)=>{e.preventDefault();setBusy(true);const f=new FormData(e.currentTarget);const id=assignment?.odev_id||uid('ODV');const selectedStudentId=String(f.get('ogrenci_id'));const selectedTeacherId=String(f.get('ogretmen_id'));const assignedDate=String(f.get('verilis_tarihi'));try{
    ...
    if(file||image){
      ...
      if(student&&teacher){
        try{
          await archiveAssignmentAttachmentsToDrive({
            assignmentId:id,
            studentId:selectedStudentId,
            studentName:student.ad_soyad,
            teacherName:teacher.ad_soyad,
            assignedDate,
            file:file&&saved.filePath?{path:saved.filePath,file,existingDriveLink:assignment?.odev_dosya_linki||null}:null,
            image:image&&saved.imagePath?{path:saved.imagePath,file:image,existingDriveLink:assignment?.odev_fotograf_linki||null}:null,
          })
  ...
  return <form className="form-grid assignment-editor" onSubmit={submit}>
    ...
    <div className="wide assignment-files">
      ...
      <div className="form-hint"><FileText size={14}/>Ekler Google Drive’da öğrencinin klasöründe saklanır. WhatsApp gönderiminde tıklanabilir bağlantı olarak paylaşılır.</div>
    </div>

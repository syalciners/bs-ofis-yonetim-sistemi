...
export function FinancePage(){
  const {data,refresh}=useAppData();const{toast}=useToast();const nav=useNavigate();const[params]=useSearchParams();const init=(params.get('tab') as Tab)||'tahsilatlar';const[tab,setTab]=useState<Tab>(init);const[add,setAdd]=useState<Tab|null>(null);const[paymentTeacher,setPaymentTeacher]=useState<string|null>(null);const[selected,setSelected]=useState<any|null>(null);const[cancelBusy,setCancelBusy]=useState(false);const[showCanceledCollections,setShowCanceledCollections]=useState(false);const[collectionLimit,setCollectionLimit]=useState(50);const[cashMovementLimit,setCashMovementLimit]=useState(50);const[showCanceledTeacherPayments,setShowCanceledTeacherPayments]=useState(false)

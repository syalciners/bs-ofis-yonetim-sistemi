...
export function ReportsPage(){
  ...
  return <div className="page-stack report-page">
    ...
    <section className="report-sheet">
      <header><img src="./bs-app-icon-192.png"/><div><h2>BS Ofis Yönetim Sistemi</h2><p>{type==='kurum'?'Aylık Kurum Özeti':type==='ogrenci'?'Öğrenci Ekstresi':'Öğretmen Hakediş Özeti'}</p></div><span>{fullDate(today)}</span></header>
      ...
      {type==='kurum'&&<>
        ...
        <h3>Ders Özeti</h3><div className="report-table"><div className="report-tr head"><span>Durum</span><span>Adet</span><span>Öğrenci Tutarı</span><span>Hakediş</span></div>{lessonStatuses.map(status=>{const rows=data.dersler.filter(x=>x.tarih?.startsWith(today.slice(0,7))&&x.ders_durumu===status);const finansal=status==='Yapıldı';return <div className="report-tr" key={status}><span>{status}</span><span>{rows.length}</span><span>{money(finansal?rows.reduce((a,x)=>a+Number(x.ogrenci_toplam_tutar||0),0):0)}</span><span>{money(finansal?rows.reduce((a,x)=>a+Number(x.ogretmen_toplam_hakedis||0),0):0)}</span></div>})}</div>
      </>}

      {type==='ogrenci'&&student&&(()=>{const s=data.ogrenciler.find(x=>x.ogrenci_id===student)!;const allLessons=data.dersler.filter(x=>x.ogrenci_id===student&&x.ders_durumu==='Yapıldı');const allPayments=data.tahsilatlar.filter(x=>x.ogrenci_id===student&&!x.iptal_mi);const lessons=allLessons.slice(0,30),payments=allPayments.slice(0,30);const totalAccrual=allLessons.reduce((a,x)=>a+Number(x.ogrenci_toplam_tutar||0),0),totalPaid=allPayments.reduce((a,x)=>a+Number(x.tutar||0),0);return <>
      ...
      {type==='ogretmen'&&teacher&&teacherPeriod&&(()=>{const t=data.ogretmenler.find(x=>x.ogretmen_id===teacher)!;const p=periods.find(x=>x.hakedis_donemi_id===teacherPeriod);if(!p)return <div className="report-placeholder">Hakediş dönemi bulunamadı.</div>;const allLessons=data.dersler.filter(x=>x.ogretmen_id===teacher&&x.ders_durumu==='Yapıldı'&&(x.tarih||'')>=p.baslangic_tarihi&&(x.tarih||'')<=p.bitis_tarihi).sort((a,b)=>String(b.tarih||'').localeCompare(String(a.tarih||'')));const allPayments=data.ogretmenOdemeleri.filter(x=>x.ogretmen_id===teacher&&x.hakedis_donemi_id===teacherPeriod&&!x.iptal_mi).sort((a,b)=>String(b.tarih||'').localeCompare(String(a.tarih||'')));const totalEarned=allLessons.reduce((a,x)=>a+Number(x.ogretmen_toplam_hakedis||0),0),totalPaid=allPayments.reduce((a,x)=>a+Number(x.tutar||0),0),remaining=totalEarned-totalPaid;return <>
        <h3>{t.ad_soyad} · {p.donem_adi}</h3><div className="report-kpis"><div><span>Yapılan Ders</span><b>{allLessons.length}</b></div><div><span>Dönem Hakedişi</span><b>{money(totalEarned)}</b></div><div><span>Dönem Ödemesi</span><b>{money(totalPaid)}</b></div><div><span>Kalan Hakediş</span><b>{money(remaining)}</b></div></div>
        <h3>Dönem Dersleri</h3><div className="report-table">{allLessons.length?allLessons.map(x=><div className="report-tr" key={x.ders_id}><span>{fullDate(x.tarih)}</span><span>{money(x.ogretmen_toplam_hakedis)}</span></div>):<div className="report-placeholder">Bu dönemde yapılmış ders yok.</div>}</div>
        <h3>Dönem Ödemeleri</h3><div className="report-table">{allPayments.length?allPayments.map(x=><div className="report-tr" key={x.ogretmen_odeme_id}><span>{fullDate(x.tarih)}</span><span>{x.odeme_yontemi}</span><span>{money(x.tutar)}</span></div>):<div className="report-placeholder">Bu dönemde ödeme kaydı yok.</div>}</div>
      </>})()}

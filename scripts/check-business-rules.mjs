...
const profileMigration = read('supabase/migrations/20260814_kullanici_kendi_profilini_guncelle_guvenli_v1.sql')
...
expectText('Kendi profilini güncelleme güvenli RPC kullanır', profileService, "supabase.rpc('kullanici_kendi_profilini_guncelle_guvenli_v1'")
expectText('Profil güncelleme RPC yalnız oturumdaki kullanıcıyı hedefler', profileMigration, 'v_uid uuid := auth.uid();')
expectText('Profil güncelleme RPC yalnız kendi auth kaydını günceller', profileMigration, 'where auth_user_id = v_uid;')
expectText('Profil güncelleme RPC anonim kullanıma kapalıdır', profileMigration, 'from public, anon;')
expectText('Profil güncelleme RPC authenticated kullanıcıya açıktır', profileMigration, 'to authenticated, service_role;')
expectText('Ayarlar ekranı profil düzenlemeyi güvenli servise bağlar', settings, 'await updateOwnProfile({ad_soyad:')
expectText('Ayarlar ekranı yetki alanlarının değiştirilemediğini açıklar', settings, 'E-posta, rol ve kullanıcı yetkisi güvenlik nedeniyle bu ekrandan değiştirilemez.')
...
rejectRegex('Kendi profilini düzenleme rol/aktif/e-posta alanını göndermez', profileService, /p_(?:rol|aktif|email)\b/i, 'Kendi profilini düzenleme servisi yetki kapsamı dışındaki alanı gönderiyor.')
rejectRegex('Kendi profilini düzenleme migrationı rol/aktif/e-posta değiştirmez', profileMigration, /\b(?:rol|aktif|email)\s*=/i, 'Kendi profilini düzenleme RPC yetki kapsamı dışındaki alanı değiştiriyor.')

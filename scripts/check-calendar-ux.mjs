...
const checks = [
  ...
  ['Tümü Öğretmen Öğrenci segment butonları kaldırılmıştır', !calendar.includes('calendar-filter-segment') && !calendar.includes('>Tümü</button>')],
  ['Başak ve Süleyman hızlı öğretmen seçimi geri gelmiştir', calendar.includes('calendar-manager-quick') && calendar.includes('calendar-quick-teacher') && calendar.includes('isManagerTeacher') && calendar.includes('teacherTone')],
  ['Öğretmen açılır seçicisi sürekli görünür', calendar.includes('teacher-person-picker') && calendar.includes('aria-label="Öğretmen seç"') && calendar.includes('{activeTeachers.map')],
  ['Öğrenci açılır seçicisi sürekli görünür', calendar.includes('student-person-picker') && calendar.includes('aria-label="Öğrenci seç"') && calendar.includes('{activeStudents.map')],
  ...
  ['Boş kişi seçimi tüm programa döner', (calendar.match(/e\.target\.value\?\{type:'(?:teacher|student)',id:e\.target\.value\}:\{type:'all'\}/g)||[]).length===2],
  ['Aktif hızlı öğretmene tekrar dokunmak tüm programa döner', calendar.includes("active?{type:'all'}:{type:'teacher',id:x.ogretmen_id}")],
  ['Takvim kişi filtresi oturumda korunur', calendar.includes("sessionStorage.setItem('bs-takvim-kisi',serializeFilter(filter))")],
  ['Tüm programda program paylaşımı açılmaz', calendar.includes("filter.type==='all'?null:{type:filter.type,id:filter.id}") && calendar.includes('disabled={!shareTarget||!lessons.length}')],
  ['Ayrı tarih aralığı satırı kaldırılmıştır', !calendar.includes('<div className="week-range">')],
  ['Tarih aralığı kişi özetiyle aynı komut bloğundadır', calendar.includes('calendar-command-summary') && calendar.includes('calendar-command-title') && calendar.includes('calendar-command-date') && calendar.includes('{shortDate(monday)} – {shortDate(sunday)}')],
  ['Hafta hazırlama aksiyonu Takvim başlığındadır', calendar.includes('calendar-title-line') && calendar.includes('calendar-title-week-action') && programCss.includes('.calendar-title-week-action')],
  ...
  ['Boş hafta yönlendirme mesajı hafta ve kişi seçimini açıklar', calendar.includes('Başka bir hafta, öğretmen veya öğrenci seçebilirsin.')],
  ...
  ['Hızlı öğretmenler iki eşit sütundur', programCss.includes('.calendar-manager-quick') && programCss.includes('grid-template-columns:repeat(2,minmax(0,1fr))')],
  ['Öğretmen ve öğrenci açılır alanları iki eşit sütundur', programCss.includes('.calendar-person-selectors') && programCss.includes('grid-template-columns:repeat(2,minmax(0,1fr))')],
  ['Eski üçlü segment CSS katmanı kaldırılmıştır', !programCss.includes('.calendar-filter-segment')],
  ['Mobilde hızlı seçim ve açılır alanlar yan yana korunur', programCss.includes('@media(max-width:520px)') && programCss.includes('.calendar-manager-quick,.calendar-person-selectors{gap:6px}')],
  ['Mobilde kişi adı ve tarih aynı kart içinde alt alta uyarlanır', programCss.includes('.calendar-command-title{align-items:flex-start;flex-direction:column')],

  ...
  return <div className="exam-add-overlay" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose() }}>
    <section className="exam-photo-sheet" role="dialog" aria-modal="true" aria-labelledby="exam-photo-title">
      ...
      <div className="exam-photo-body">
        ...
        <input ref={inputRef} className="exam-photo-file" type="file" accept="image/*" capture="environment" onChange={event => void process(event)}/>

        {!preview && <button type="button" className="exam-photo-capture" onClick={choose} disabled={busy}>
          <Camera/><div><b>Fotoğraf çek veya seç</b><span>Sonuç tablosunu mümkün olduğunca düz ve net kadraja alın.</span></div>
        </button>}

        {preview && <div className="exam-photo-preview-wrap">
          <img className="exam-photo-preview" src={preview} alt="Seçilen deneme sonucu"/>
          <button type="button" className="exam-photo-replace" onClick={choose} disabled={busy}><ImagePlus/> Başka fotoğraf</button>
        </div>}

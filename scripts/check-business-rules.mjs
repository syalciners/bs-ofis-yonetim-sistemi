...
const collectionDeleteMigration = read('supabase/migrations/20260816010000_tahsilat_kalici_silme_v1.sql')
...
expectText('Canlı dağıtım Öğretmenler UX kontrolünü çalıştırır', deployWorkflow, 'node scripts/check-teachers-ux.mjs')
expectText('Canlı dağıtım Öğrenciler UX kontrolünü çalıştırır', deployWorkflow, 'node scripts/check-students-ux.mjs')
expectText('Canlı dağıtım detay ekranı UX kontrolünü çalıştırır', deployWorkflow, 'node scripts/check-detail-ux.mjs')
expectText('Canlı dağıtım Akıllı Programlama kontrolünü çalıştırır', deployWorkflow, 'node scripts/check-smart-scheduling.mjs')
expectText('Canlı dağıtım Ayarlar güvenlik kontrolünü çalıştırır', deployWorkflow, 'node scripts/check-settings-ux.mjs')
expectText('Canlı dağıtım Ödev ve WhatsApp kontrolünü çalıştırır', deployWorkflow, 'node scripts/check-assignments-ux.mjs')
...
expectText('Kasa hareketleri ilk açılışta 50 kayıtla sınırlandırılır', finance, 'const[cashMovementLimit,setCashMovementLimit]=useState(50)')
expectText('Kasa hareketleri yalnız görünür kayıtları kademeli çizer', finance, 'const shownCashMovements=data.kasaHareketleri.slice(0,cashMovementLimit)')
expectText('Kasa hareketleri eski kayıtları 50şer açar', finance, 'setCashMovementLimit(limit=>limit+50)')
expectText('Kasa hareketi sayacı görünür ve toplam kaydı gösterir', finance, '{shownCashMovements.length} / {data.kasaHareketleri.length} kayıt')
...
expectText('Finans ekranı iptal giderleri varsayılan listeden ayırır', finance, 'const visibleExpenses=showCanceledExpenses?data.giderler:data.giderler.filter(x=>!x.iptal_mi)')
expectText('Giderler iptal kayıtlarını isteğe bağlı gösterir', finance, 'aria-pressed={showCanceledExpenses}')
expectText('Gider listesi ilk açılışta 50 kayıtla sınırlandırılır', finance, 'const[expenseLimit,setExpenseLimit]=useState(50)')
expectText('Gider listesi yalnız görünür kayıtları kademeli çizer', finance, 'const shownExpenses=visibleExpenses.slice(0,expenseLimit)')
expectText('İptal görünürlüğü değişince gider sınırı sıfırlanır', finance, 'setShowCanceledExpenses(x=>!x);setExpenseLimit(50)')
expectText('İptal giderler gizliyken açıklayıcı boş durum gösterilir', finance, 'İptal edilen giderler varsayılan listede gizleniyor.')
...
expectText('PWA 192 piksel eğitim ikonunu kullanır', viteConfig, "{ src: 'bs-egitim-icon-192-v2.png', sizes: '192x192', type: 'image/png' }")
expectText('PWA 512 piksel eğitim ikonunu kullanır', viteConfig, "{ src: 'bs-egitim-icon-512-v2.png', sizes: '512x512', type: 'image/png' }")
...
expectText('Apple ana ekranı açık zeminli eğitim ikonunu kullanır', indexHtml, '<link rel="apple-touch-icon" href="./bs-egitim-apple-touch-v2.png" />')
expectText('Tarayıcı faviconu açık zeminli eğitim ikonunu kullanır', indexHtml, '<link rel="icon" type="image/png" sizes="32x32" href="./bs-egitim-favicon-32-v2.png" />')
...
expectText('İptal tahsilat yalnız güvenli kalıcı silme RPC ile kaldırılır', cancelService, "run('tahsilat_sil_guvenli_v1'")
expectText('Kalıcı silme düğmesi yalnız iptal tahsilatta görünür', finance, "selected.type==='tahsilat'&&selected.row.iptal_mi")
expectText('Kalıcı silme geri alınamaz uyarısı verir', finance, 'Bu işlem geri alınamaz.')
expectText('Kalıcı silme sonrası Finans senkronizasyonu kullanıcıya bildirilir', finance, 'Finans senkronizasyonu başlatıldı.')
expectText('Kalıcı silme RPC aktif tahsilatı reddeder', collectionDeleteMigration, 'Aktif tahsilat kalıcı olarak silinemez. Önce kaydı iptal edin.')
expectText('Kalıcı silme RPC kasa hareketinde bire bir eşleşme ister', collectionDeleteMigration, 'Tahsilata bağlı birden fazla kasa hareketi bulundu.')
expectText('Kalıcı silme RPC Finans senkronizasyonunu tetikler', collectionDeleteMigration, 'private.finans_v18_sync_tetikle()')
expectText('Kalıcı silme RPC yalnız authenticated rolüne açılır', collectionDeleteMigration, 'grant execute on function public.tahsilat_sil_guvenli_v1(text)\nto authenticated;')
...
rejectRegex('Eski V1–V4 hafta üretim RPC çağrıları frontend aktif kodunda bulunmaz', allSource, /supabase\.rpc\('haftalik_dersleri_olustur_guvenli_v[1-4]'/, 'Frontend içinde eski V1–V4 haftalık ders üretim RPC çağrısı bulundu.')
rejectRegex('Eski V1 hafta durum RPC çağrısı frontend aktif kodunda bulunmaz', allSource, /supabase\.rpc\('haftalik_ders_uretim_durumu_v1'/, 'Frontend içinde eski V1 hafta durum RPC çağrısı bulundu.')

...
const checks = [
  ...
  ['Kişi filtresi Tümü Öğretmen Öğrenci segmentidir', calendar.includes('calendar-filter-segment') && calendar.includes(">Tümü</button>") && calendar.includes(">Öğretmen</button>") && calendar.includes(">Öğrenci</button>")],
  ['Eski yönetici ve ikili seçim blokları Takvimden kaldırılmıştır', !calendar.includes('teacher-manager-grid') && !calendar.includes('teacher-secondary-compact')],
  ['Öğretmen seçici yalnız Öğretmen modunda görünür', calendar.includes("filter.type==='teacher'&&<label className=\"calendar-person-picker teacher-person-picker\"") && calendar.includes('aria-label="Öğretmen seç"')],
  ['Öğrenci seçici yalnız Öğrenci modunda görünür', calendar.includes("filter.type==='student'&&<label className=\"calendar-person-picker student-person-picker\"") && calendar.includes('aria-label="Öğrenci seç"')],
  ...
  ['Öğrenci seçimi yalnız öğrencinin derslerini gösterir', calendar.includes("filter.type==='student'&&Boolean(filter.id)&&x.ogrenci_id===filter.id")],
  ['Öğretmen seçimi yalnız öğretmenin derslerini gösterir', calendar.includes("filter.type==='teacher'&&Boolean(filter.id)&&x.ogretmen_id===filter.id")],
  ['Tümü tek dokunuşta erişilebilir kalır', calendar.includes("chooseMode('all')") && calendar.includes("chooseFilter({type:'all'})")],
  ['Takvim kişi filtresi oturumda korunur', calendar.includes("sessionStorage.setItem('bs-takvim-kisi',serializeFilter(filter))")],
  ['Kişi türü seçilip isim seçilmeden program paylaşımı açılmaz', calendar.includes("filter.type==='all'||!filter.id?null") && calendar.includes('disabled={!shareTarget||!lessons.length}')],
  ...
  ['Kişi seçilmemiş modda yönlendirici boş durum mesajı vardır', calendar.includes('Öğretmen seçerek programı görüntüleyebilirsin.') && calendar.includes('Öğrenci seçerek programı görüntüleyebilirsin.')],
  ...
  ['Kompakt kişi segmenti üç eşit sütundur', programCss.includes('grid-template-columns:repeat(3,minmax(0,1fr))')],
  ['Koşullu kişi seçici tek satır ve tam genişliktir', programCss.includes('.calendar-person-picker') && programCss.includes('grid-template-columns:auto minmax(0,1fr)')],
  ...
  ['Mobil kişi seçici yatay taşma yaratmadan tek sütuna iner', programCss.includes('@media(max-width:520px)') && programCss.includes('.calendar-person-picker{grid-template-columns:1fr')],

import { Banknote, CircleDollarSign, GraduationCap, Landmark, Plus, Receipt, ReceiptText, Users, WalletCards } from 'lucide-react'
...
export function FinancePage(){
  const {data}=useAppData();const nav=useNavigate();const[params]=useSearchParams();const init=(params.get('tab') as Tab)||'tahsilatlar';const[tab,setTab]=useState<Tab>(init);const[add,setAdd]=useState<Tab|null>(null);const[paymentTeacher,setPaymentTeacher]=useState<string|null>(null);const[selected,setSelected]=useState<any|null>(null)
  ...
  return <div className="page-stack">
    ...
    {tab==='tahsilatlar'&&<>
      <div className="section-heading"><div><h2>Son Tahsilatlar</h2><span>{data.tahsilatlar.length} kayıt</span></div><button className="text-btn" onClick={()=>nav('/ogrenciler?filtre=borclu')}>Borçlu Öğrenciler</button></div>
      <section className="finance-list">{data.tahsilatlar.length?data.tahsilatlar.map(x=><button className="finance-card income" key={x.tahsilat_id} onClick={()=>setSelected({type:'tahsilat',row:x})}><div className="finance-icon"><CircleDollarSign/></div><div><strong>{studentName(data,x.ogrenci_id)}</strong><small>{fullDate(x.tarih)} · {x.odeme_yontemi||'—'}{x.aciklama?` · ${x.aciklama}`:''}</small></div><b>{money(x.tutar)}</b></button>):<div className="calm-empty"><Users/><b>Henüz tahsilat yok.</b><span>İlk tahsilatı “Tahsilat Al” ile kaydedebilirsin.</span></div>}</section>
    </>}
    ...
    {tab==='ogretmen'&&<>
      ...
      <section className="finance-list">{data.ogretmenOdemeleri.length?data.ogretmenOdemeleri.map(x=><button className="finance-card expense" key={x.ogretmen_odeme_id} onClick={()=>setSelected({type:'ogretmen',row:x})}><div className="finance-icon"><Banknote/></div><div><strong>{teacherName(data,x.ogretmen_id)}</strong><small>{fullDate(x.tarih)} · {data.hakedisDonemleri.find(d=>d.hakedis_donemi_id===x.hakedis_donemi_id)?.donem_adi||'—'} · {x.odeme_yontemi||'—'}</small></div><b>− {money(x.tutar)}</b></button>):<div className="calm-empty"><GraduationCap/><b>Henüz öğretmen ödemesi yok.</b><span>Ödeme bekleyen öğretmene dokunarak doğrudan ödeme başlatabilirsin.</span></div>}</section>
    </>}

    {tab==='giderler'&&<>
      <div className="section-heading"><div><h2>Giderler</h2><span>bu ay {money(kpis.expenses)}</span></div></div>
      <section className="finance-list">{data.giderler.length?data.giderler.map(x=><button className="finance-card expense" key={x.gider_id} onClick={()=>setSelected({type:'gider',row:x})}><div className="finance-icon"><Receipt/></div><div><strong>{expenseName(data,x.kategori_id)}</strong><small>{fullDate(x.tarih)} · {x.odeme_yontemi||'—'}{x.aciklama?` · ${x.aciklama}`:''}</small></div><b>− {money(x.tutar)}</b></button>):<div className="calm-empty"><ReceiptText/><b>Henüz gider kaydı yok.</b><span>Gider eklendiğinde kasa çıkışı otomatik oluşur.</span></div>}</section>
    </>}

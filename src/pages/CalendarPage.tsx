...
export function CalendarPage(){
  ...
  const allWeekLessons=useMemo(()=>{if(!data)return[];return data.dersler.filter(x=>(x.tarih||'')>=monday&&(x.tarih||'')<end).sort((a,b)=>String(a.tarih||'').localeCompare(String(b.tarih||''))||String(a.baslangic_saati||'').localeCompare(String(b.baslangic_saati||'')))},[data,monday,end])
  ...
  const openWeekPdf=()=>{if(!allWeekLessons.length){toast('Bu haftada PDF oluşturulacak ders yok.','error');return}if(!openWeeklyProgramPdf(data,allWeekLessons,monday,addDays(monday,6)))toast('PDF penceresi açılamadı. Tarayıcı açılır pencere iznini kontrol et.','error')}

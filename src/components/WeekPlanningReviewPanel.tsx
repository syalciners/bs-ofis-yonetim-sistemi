...
export function WeekPlanningReviewPanel({ review, onChange, onClose, onCreate }: {review:WeekPlanningReview;onChange:(r:WeekPlanningReview)=>void;onClose:()=>void;onCreate:()=>void}) {
  ...
  const apply=async(issue:WeekPlanningIssue,s:ProgramSuggestion)=>{
    ...
    try{
      ...
      onChange(next)
      toast(next.uygun?'Çakışmalar giderildi. Haftayı artık oluşturabilirsiniz.':'Öneri uygulandı; kalan çakışmaları kontrol edin.')
    }catch(e:any){toast(e.message||String(e),'error')}finally{setApplying('')}
  }

  if(review.uygun)return <div className="week-review-ready"><CalendarCheck2/><div><b>Program artık uygun.</b><span>Seçilen hafta ve sonraki hafta çakışmasız görünüyor.</span></div><button className="primary-btn" type="button" onClick={onCreate}>Haftayı Şimdi Oluştur</button><button className="secondary-btn" type="button" onClick={onClose}>Kapat</button></div>

  return <div className="week-review-list">
    <div className="week-review-summary"><AlertTriangle/><span><b>{review.sorun_sayisi} ders düzenleme istiyor.</b><small>Hiçbir ders oluşturulmadı. Bir öneri seçerek yalnız ilgili haftayı düzeltebilirsiniz.</small></span></div>
    {review.sorunlar.map((issue,index)=>{
      ...
      return <section className="week-review-card" key={`${issue.program_id}-${issue.tarih}-${index}`}>
        ...
        {!issue.onerilen_derslikler?.length&&!issue.onerilen_saatler?.length&&<div className="form-hint">Yakın uygun seçenek bulunamadı. Sabit programdan gün, saat veya öğretmeni değiştirin.</div>}
      </section>
    })}

...
const parseFilter=(value:string|null):CalendarFilter|null=>{
  ...
  if(value.startsWith('teacher:'))return{type:'teacher',id:value.slice(8)||undefined}
  if(value.startsWith('student:'))return{type:'student',id:value.slice(8)||undefined}
  return null
}
...
export function CalendarPage(){
  ...
  const lessons=useMemo(()=>{if(!data)return[];return data.dersler.filter(x=>{
    const matchesPerson=filter.type==='all'||(filter.type==='teacher'&&Boolean(filter.id)&&x.ogretmen_id===filter.id)||(filter.type==='student'&&Boolean(filter.id)&&x.ogrenci_id===filter.id)
    return matchesPerson&&(x.tarih||'')>=monday&&(x.tarih||'')<end
  }).sort((a,b)=>String(a.tarih||'').localeCompare(String(b.tarih||''))||String(a.baslangic_saati||'').localeCompare(String(b.baslangic_saati||'')))},[data,filter,monday,end])
  useEffect(()=>{sessionStorage.setItem('bs-takvim-kisi',serializeFilter(filter));sessionStorage.setItem('bs-takvim-ogretmen',filter.type==='teacher'&&filter.id?filter.id:'tum')},[filter])
  ...
  const filterLabel=filter.type==='all'?'Tüm Program':filter.type==='teacher'?(activeTeachers.find(x=>x.ogretmen_id===filter.id)?.ad_soyad||'Öğretmen seç'):(activeStudents.find(x=>x.ogrenci_id===filter.id)?.ad_soyad||'Öğrenci seç')
  const shareTarget:ProgramShareTarget|null=filter.type==='all'||!filter.id?null:{type:filter.type,id:filter.id}
  const chooseFilter=(next:CalendarFilter)=>{setFilter(next);setShareOpen(false)}
  const chooseMode=(type:'all'|'teacher'|'student')=>{if(type==='all')chooseFilter({type:'all'});else if(filter.type!==type)chooseFilter({type})}
  const emptyHint=filter.type==='teacher'&&!filter.id?'Öğretmen seçerek programı görüntüleyebilirsin.':filter.type==='student'&&!filter.id?'Öğrenci seçerek programı görüntüleyebilirsin.':'Başka bir hafta veya kişi seçebilirsin.'
  ...
  return <div className="page-stack calendar-v2">
    ...
    <section className="calendar-person-filter" aria-label="Takvim kişi filtresi">
      <div className="calendar-filter-segment" role="group" aria-label="Program türü">
        <button className={filter.type==='all'?'active':''} onClick={()=>chooseMode('all')}>Tümü</button>
        <button className={filter.type==='teacher'?'active':''} onClick={()=>chooseMode('teacher')}>Öğretmen</button>
        <button className={filter.type==='student'?'active':''} onClick={()=>chooseMode('student')}>Öğrenci</button>
      </div>
      {filter.type==='teacher'&&<label className="calendar-person-picker teacher-person-picker">
        <span>Öğretmen</span>
        <select aria-label="Öğretmen seç" value={filter.id||''} onChange={e=>chooseFilter({type:'teacher',id:e.target.value||undefined})}>
          <option value="">Öğretmen seç</option>
          {activeTeachers.map(x=><option key={x.ogretmen_id} value={x.ogretmen_id}>{x.ad_soyad}</option>)}
        </select>
      </label>}
      {filter.type==='student'&&<label className="calendar-person-picker student-person-picker">
        <span>Öğrenci</span>
        <select aria-label="Öğrenci seç" value={filter.id||''} onChange={e=>chooseFilter({type:'student',id:e.target.value||undefined})}>

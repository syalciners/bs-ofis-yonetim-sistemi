...
export function TeachersPage(){
  ...
  return <div className="page-stack teachers-v2">
    ...
    <Sheet open={!!selected&&!payment&&!edit} title={selected?.ad_soyad||'Öğretmen'} subtitle="Öğretmen profili" onClose={()=>setSelected(null)}>{selected&&(()=>{const next=nextLessonForTeacher(data,selected.ogretmen_id),phone=selected.telefon||'',branches=branchNames(selected.ogretmen_id),manager=isManagerTeacher(selected.ad_soyad),title=manager?`Yönetici - ${teachingTitle(branches)}`:teachingTitle(branches),balance=Math.max(teacherBalance(data,selected.ogretmen_id),0),monthPrefix=firstOfMonth().slice(0,7),monthLessons=data.dersler.filter(x=>x.ogretmen_id===selected.ogretmen_id&&x.ders_durumu==='Yapıldı'&&x.tarih?.startsWith(monthPrefix)).length,recent=data.dersler.filter(x=>x.ogretmen_id===selected.ogretmen_id).slice(0,8);return <div className="detail-stack profile-detail-stack">
      ...
      <section className="profile-focus-grid"><div className="profile-focus-card money-focus"><span>Güncel Kalan Hakediş</span><strong className={balance>0?'danger-text':''}>{money(balance)}</strong><small>{balance>0?'Ödeme bekliyor':'Ödeme dengede'}</small></div><div className="profile-focus-card"><span>Sıradaki Ders</span><strong>{next?`${shortDate(next.tarih)} ${time(next.baslangic_saati)}`:'—'}</strong><small>{next?studentName(data,next.ogrenci_id):'Planlı ders yok'}</small></div><div className="profile-focus-card"><span>Bu Ay</span><strong>{monthLessons}</strong><small>yapılan ders</small></div></section>
      <div className="quick-detail-actions profile-actions"><button className="primary-btn" onClick={()=>setPayment(selected.ogretmen_id)}><WalletCards size={17}/>Ödeme Yap</button><button className="secondary-btn" onClick={()=>{nav(`/takvim?ogretmen=${encodeURIComponent(selected.ogretmen_id)}`);setSelected(null)}}><CalendarDays size={17}/>Takvim</button><button className="secondary-btn" onClick={()=>{nav(`/ogretmen-odemeleri?ogretmen=${encodeURIComponent(selected.ogretmen_id)}`);setSelected(null)}}><WalletCards size={17}/>Ödemeler</button></div>
      <button className="profile-more-toggle" onClick={()=>setMore(v=>!v)}><MoreHorizontal size={16}/><span>{more?'İletişim işlemlerini gizle':'İletişim ve düzenleme'}</span></button>

...
const checks=[
  ...
  ['Takvim butonu sonrası tarih ve Bu Hafta alanı eşit iki sütundur',read('src/program-week-layout.css').includes('grid-template-columns:auto minmax(0,1fr) minmax(0,1fr)')&&read('src/program-week-layout.css').includes('grid-template-columns:26px minmax(0,1fr) 26px')&&read('src/program-week-layout.css').includes('grid-template-columns:80px minmax(0,1fr) minmax(0,1fr)')&&read('src/program-week-layout.css').includes('grid-template-columns:72px minmax(0,1fr) minmax(0,1fr)')],
  ...
  ['Sabit Program Takvimi derslik sütunlarını veri kaynağından oluşturur',fixed.includes("import { calendarRoomColumns } from '../lib/calendarRooms'")&&fixed.includes('calendarRoomColumns(data.derslikler,selectedPrograms.map(x=>x.derslik_id))')&&!fixed.includes('const ROOM_COLUMNS=')],

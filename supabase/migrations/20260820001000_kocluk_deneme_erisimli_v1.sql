-- BS Koçluk Deneme Merkezi Premium V1
-- Yönetici veya öğrencinin atanmış Koç'u aynı güvenli veri çekirdeğine atomik deneme sonucu yazabilir.

create or replace function public.kocluk_deneme_tam_kaydet_erisimli_v1(
  p_deneme_id text,
  p_ogrenci_id text,
  p_sinav_turu text,
  p_deneme_adi text,
...
begin
  ...
  if not exists(select 1 from public.kocluk_ogrenci_profilleri where ogrenci_id = p_ogrenci_id and durum = 'Aktif') then raise exception 'Öğrencinin aktif koçluk profili bulunamadı.'; end if;
  if v_sinav_turu is null or v_sinav_turu not in ('LGS','TYT','AYT','Diğer') then raise exception 'Geçerli sınav türü zorunludur.'; end if;
  ...
  if p_bolumler is null or jsonb_typeof(p_bolumler) <> 'array' or jsonb_array_length(p_bolumler) = 0 then raise exception 'En az bir ders / bölüm sonucu girilmelidir.'; end if;

  perform pg_advisory_xact_lock(hashtextextended('kocluk-deneme|' || p_ogrenci_id || '|' || p_deneme_tarihi::text || '|' || lower(v_sinav_turu) || '|' || lower(v_deneme_adi), 0));

  if v_id is null then
    select d.deneme_id into v_id
    from public.kocluk_deneme_sinavlari d
    where d.ogrenci_id = p_ogrenci_id
      and d.deneme_tarihi = p_deneme_tarihi
      and lower(d.sinav_turu) = lower(v_sinav_turu)
      and lower(trim(d.deneme_adi)) = lower(trim(v_deneme_adi))
      and d.onay_durumu <> 'İptal'
    order by d.guncellenme_zamani desc nulls last
    limit 1;
  end if;
  if v_id is null then v_id := 'DNM-' || upper(substr(md5(p_ogrenci_id || '|' || p_deneme_tarihi::text || '|' || lower(v_sinav_turu) || '|' || lower(v_deneme_adi)), 1, 10)); end if;
  ...
  insert into public.kocluk_deneme_sinavlari(
    deneme_id, ogrenci_id, sinav_turu, deneme_adi, deneme_tarihi, yayinevi, veri_kaynagi,
    yanlis_boleni, puan, siralama, yuzdelik, katilimci_sayisi, sure_dakika, notlar,
    onay_durumu, olusturan, guncelleyen
  ) values (
  ...
  on conflict (deneme_id) do update set
    ogrenci_id=excluded.ogrenci_id, sinav_turu=excluded.sinav_turu, deneme_adi=excluded.deneme_adi,
    deneme_tarihi=excluded.deneme_tarihi, yayinevi=excluded.yayinevi, veri_kaynagi=excluded.veri_kaynagi,
    yanlis_boleni=excluded.yanlis_boleni, puan=excluded.puan, siralama=excluded.siralama,
    yuzdelik=excluded.yuzdelik, katilimci_sayisi=excluded.katilimci_sayisi, sure_dakika=excluded.sure_dakika,
    notlar=excluded.notlar, onay_durumu='Taslak', guncellenme_zamani=now(), guncelleyen=auth.uid();

  delete from public.kocluk_deneme_bolum_sonuclari where deneme_id = v_id;

  for v_row in select value from jsonb_array_elements(p_bolumler) as t(value) loop
    ...
    if v_dogru + v_yanlis + v_bos > v_soru_sayisi then raise exception '% için doğru + yanlış + boş toplamı soru sayısını aşamaz.', v_bolum_adi; end if;
    ...
    insert into public.kocluk_deneme_bolum_sonuclari(
      ...
      'DNS-' || upper(substr(replace(gen_random_uuid()::text, '-', ''), 1, 10)), v_id, v_bolum_adi,
      v_sira_no, v_dogru, v_yanlis, v_bos, v_soru_sayisi, v_net, auth.uid(), auth.uid()
    );
  ...
  return jsonb_build_object(
    'basarili', true, 'deneme_id', v_id, 'yeni', v_yeni, 'tekrar', not v_yeni,
    'onay_durumu', 'Onaylandı', 'bolum_sayisi', v_bolum_sayisi,
    'toplam_dogru', v_toplam_dogru, 'toplam_yanlis', v_toplam_yanlis,
    'toplam_bos', v_toplam_bos, 'toplam_net', v_toplam_net
  );
end;
$$;

revoke execute on function public.kocluk_deneme_tam_kaydet_erisimli_v1(text,text,text,text,date,text,text,numeric,numeric,integer,numeric,integer,integer,text,jsonb) from public, anon;
grant execute on function public.kocluk_deneme_tam_kaydet_erisimli_v1(text,text,text,text,date,text,text,numeric,numeric,integer,numeric,integer,integer,text,jsonb) to authenticated;

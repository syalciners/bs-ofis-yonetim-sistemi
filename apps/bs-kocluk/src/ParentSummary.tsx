...
export function ParentSummary({ studentId, studentName, onClose, onLogged }: { studentId: string; studentName: string; onClose: () => void; onLogged?: () => void }) {
  ...
  const prepare = async (knownParentName?: string) => {
    ...
    try {
      ...
      if (knownParentName == null) {
        const { data: contact, error: contactError } = await supabase.from('ogrenciler').select('veli_adi,veli_telefon').eq('ogrenci_id', studentId).maybeSingle()
        if (contactError) throw contactError
        name = String(contact?.veli_adi || '')
        setParentName(name)
        setParentPhone(String(contact?.veli_telefon || ''))
      }
      const { data, error: functionError } = await supabase.functions.invoke('kocluk-veli-ozeti-v1', { body: { ogrenci_id: studentId } })
  ...
  const logCommunication = async (channel: 'Kopyalama' | 'WhatsApp') => {
    ...
    setHistoryWarning(null)
    try {
      const { data, error: logError } = await supabase.rpc('kocluk_veli_iletisim_kaydet_v1', { p_ogrenci_id: studentId, p_kanal: channel, p_icerik: message.trim() })

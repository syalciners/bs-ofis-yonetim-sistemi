...
const checks=[
  ...
  ['Drive arşivi normal Ödevler akışında etkindir',page.includes('const driveArchiveEnabled=true')&&page.includes('driveArchiveEnabled={driveArchiveEnabled}')&&!page.includes("searchParams.get('drive_test')==='1'")],
  ['Ödev formu Drive arşivini kullanıcı dilinde açıklar',editor.includes('Ekler önce güvenli depoya alınır, ardından öğrencinin Drive klasörüne arşivlenir.')&&!editor.includes('Drive test modu açık')],
  ['Ödev formu Drive arşiv akışını çağırır',editor.includes('archiveAssignmentAttachmentsToDrive')&&editor.includes('if(driveArchiveEnabled)')],

(function(){
  ...
  function stilEkle(){
    ...
    s.textContent=`
      .bsogr-clickable{cursor:pointer!important;position:relative;padding-right:40px!important;touch-action:manipulation;-webkit-tap-highlight-color:rgba(37,99,235,.08)}
      .bsogr-clickable:active{background:#eff6ff!important}
      .bsogr-chevron{position:absolute;right:14px;top:50%;transform:translateY(-50%);color:#94a3b8;font-size:22px;pointer-events:none}
      ...
      .bsogr-sheet{width:min(780px,100%);max-height:91dvh;overflow:auto;background:#f8fafc;border-radius:26px 26px 0 0;padding:18px 18px calc(24px + env(safe-area-inset-bottom));box-shadow:0 -24px 70px rgba(15,23,42,.2)}
      ...
      .bsogr-duzenle{height:40px;display:inline-flex;align-items:center;justify-content:center;gap:7px;padding:0 14px;border:1px solid #bfdbfe;border-radius:12px;background:#eff6ff;color:#1d4ed8;font-size:11px;font-weight:800;cursor:pointer;touch-action:manipulation}
      .bsogr-duzenle:active{transform:scale(.98);background:#dbeafe}
      .bsogr-duzenle svg{width:15px;height:15px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}
      .bsogr-kapat{width:40px;height:40px;border:1px solid var(--kenar);border-radius:12px;background:#fff;color:var(--ikincil);font-size:23px;cursor:pointer}
      ...
      .bsogr-edit-sheet{width:min(720px,100%);max-height:94dvh;overflow:auto;background:#f8fafc;border-radius:26px 26px 0 0;padding:18px 18px calc(24px + env(safe-area-inset-bottom));box-shadow:0 -24px 70px rgba(15,23,42,.22)}
      ...
      .bsogr-alan input,.bsogr-alan select,.bsogr-alan textarea{width:100%;border:1px solid var(--kenar);border-radius:12px;background:#fff;color:var(--yazi);font:inherit;font-size:13px;outline:none;padding:11px 12px}.bsogr-alan input,.bsogr-alan select{height:44px}.bsogr-alan textarea{min-height:88px;resize:vertical;line-height:1.45}
      ...
      .bsogr-form-buton{display:flex;justify-content:flex-end;gap:8px;margin-top:15px}.bsogr-iptal,.bsogr-kaydet{height:44px;padding:0 16px;border-radius:12px;font-size:12px;font-weight:800;cursor:pointer}.bsogr-iptal{border:1px solid var(--kenar);background:#fff;color:var(--ikincil)}.bsogr-kaydet{border:1px solid var(--mavi);background:var(--mavi);color:#fff;min-width:112px}.bsogr-kaydet[disabled]{opacity:.55;cursor:default}
      ...
      @media(max-width:700px){.bsogr-sheet,.bsogr-edit-sheet{padding:15px 11px calc(18px + env(safe-area-inset-bottom));border-radius:21px 21px 0 0}.bsogr-baslik{font-size:18px}.bsogr-duzenle{height:38px;padding:0 11px;font-size:10px}.bsogr-kapat{width:38px;height:38px}.bsogr-kpi{grid-template-columns:repeat(2,minmax(0,1fr))}.bsogr-grid{grid-template-columns:1fr}.bsogr-satir{grid-template-columns:54px minmax(0,1fr) auto;padding:10px}.bsogr-form{grid-template-columns:1fr;gap:9px}.bsogr-alan.tam{grid-column:auto}.bsogr-form-buton{position:sticky;bottom:0;margin:14px -2px -2px;padding:10px 2px 2px;background:linear-gradient(to top,#f8fafc 78%,rgba(248,250,252,0))}.bsogr-iptal,.bsogr-kaydet{flex:1}}
    `;
  ...
  function modalleriHazirla(){
    ...
    if(!document.getElementById('bsOgrenciEditModal')){
      ...
      e.innerHTML=`<div class="bsogr-edit-sheet"><div class="bsogr-ust"><div><div class="bsogr-baslik">Öğrenci Bilgilerini Düzenle</div><div class="bsogr-edit-alt">Değişiklikler BS Eğitim uygulaması üzerinden kaydedilir.</div></div><button id="bsOgrenciEditKapat" class="bsogr-kapat" type="button">×</button></div><form id="bsOgrenciForm" autocomplete="off"><div class="bsogr-form"><div class="bsogr-alan tam"><label for="bsOgrAdSoyad">Ad Soyad</label><input id="bsOgrAdSoyad" required maxlength="160"></div><div class="bsogr-alan"><label for="bsOgrVeliAdi">Veli Adı</label><input id="bsOgrVeliAdi" maxlength="160"></div><div class="bsogr-alan"><label for="bsOgrDurum">Durum</label><select id="bsOgrDurum"><option value="Aktif">Aktif</option><option value="Pasif">Pasif</option></select></div><div class="bsogr-alan"><label for="bsOgrVeliTelefon">Veli Telefon</label><input id="bsOgrVeliTelefon" type="tel" inputmode="tel" maxlength="40"></div><div class="bsogr-alan"><label for="bsOgrTelefon">Öğrenci Telefon</label><input id="bsOgrTelefon" type="tel" inputmode="tel" maxlength="40"></div><div class="bsogr-alan"><label for="bsOgrEmail">E-posta</label><input id="bsOgrEmail" type="email" inputmode="email" maxlength="180"></div><div class="bsogr-alan"><label for="bsOgrKayit">Kayıt Tarihi</label><input id="bsOgrKayit" type="date"></div><div class="bsogr-alan tam"><label for="bsOgrNot">Not</label><textarea id="bsOgrNot" maxlength="2000"></textarea></div></div><div id="bsOgrMesaj" class="bsogr-mesaj"></div><div class="bsogr-form-buton"><button id="bsOgrVazgec" class="bsogr-iptal" type="button">Vazgeç</button><button id="bsOgrKaydet" class="bsogr-kaydet" type="submit">Kaydet</button></div></form></div>`;
  ...
  function editKapat(){document.getElementById('bsOgrenciEditModal')?.classList.remove('acik');mesaj('','');}
  ...
  async function detayAc(o){
    ...
    document.getElementById('bsOgrenciMeta').innerHTML=[o.durum||'',o.veli_adi?('Veli: '+o.veli_adi):'',o.ogrenci_telefon||o.veli_telefon||'',o.email||''].filter(Boolean).map(x=>'<span>'+htmlKacir(x)+'</span>').join('');
    icerik.innerHTML='<div class="bsogr-bos">Ders ve tahsilat bilgileri yükleniyor…</div>';modal.classList.add('acik');
    try{
      const sonuc=await BSOgrenciServisi.ogrenciDetayGetir(o.ogrenci_id);
  ...
  async function duzenleAc(){
    ...
    const modal=document.getElementById('bsOgrenciEditModal'),b=document.getElementById('bsOgrKaydet');
    modal.classList.add('acik');mesaj('','');b.disabled=true;b.textContent='Yükleniyor…';
    try{
  ...
  async function kaydet(e){
    ...
    const b=document.getElementById('bsOgrKaydet');b.disabled=true;b.textContent='Kaydediliyor…';mesaj('','');
    try{
      await BSOgrenciServisi.ogrenciGuncelle(aktifOgrenciId,payload);
      mesaj('Öğrenci bilgileri kaydedildi.','basari');if(typeof toast==='function')toast('Öğrenci bilgileri kaydedildi.');setTimeout(()=>window.location.reload(),550);
    }catch(err){console.error('Öğrenci modülü güncelle:',err);mesaj('Kayıt yapılamadı. Yetki veya bağlantı kontrol edilmeli.','hata');b.disabled=false;b.textContent='Kaydet';}
  }

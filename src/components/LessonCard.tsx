...
export function LessonCard({ lesson, onClick, compact=false }: { lesson:Ders;onClick:()=>void;compact?:boolean }) {
  ...
  const branch=branchName(data,lesson.brans_id)
  const room=roomName(data,lesson.derslik_id)
  const status=(lesson.ders_durumu||'Planlandı').toLocaleLowerCase('tr-TR').replaceAll(' ','-').replaceAll('ı','i').replaceAll('ş','s').replaceAll('ğ','g').replaceAll('ü','u').replaceAll('ö','o').replaceAll('ç','c')

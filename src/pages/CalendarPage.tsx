...
export function CalendarPage(){
  ...
  const openWeekPdf=()=>{if(!lessons.length){toast('Seçili programda PDF oluşturulacak ders yok.','error');return}if(!openWeeklyProgramPdf(data,lessons,monday,addDays(monday,6),filterLabel))toast('PDF penceresi açılamadı. Tarayıcı açılır pencere iznini kontrol et.','error')}
  ...
  return <div className="page-stack calendar-v2">
    ...
    <section className="calendar-command-bar">
      <div className="calendar-command-summary"><div className="calendar-command-heading"><b>{filterLabel}</b><button className="secondary-btn calendar-pdf-btn" disabled={!lessons.length} onClick={openWeekPdf} title="Seçili programı PDF olarak al"><FileDown size={16}/>PDF Al</button></div><span>{lessons.length} ders · {weekProgramCount} sabit program dersi</span></div>

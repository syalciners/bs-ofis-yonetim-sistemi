...
const checks=[
  ...
  ['Takvim 30 dakikalık hücreleri kompakt kullanır',daily.includes('const SLOT_HEIGHT=42')&&css.includes('.daily-room-grid{min-width:650px')&&css.includes('repeat(var(--room-count),minmax(116px,1fr))')],
  ...
  ['Derslik sütunları sürükleme hedefi olarak işaretlidir',daily.includes('data-room-id={column.id}')&&fixed.includes('data-room-id={column.id}')&&daily.includes("target?'drag-target':''")&&css.includes('.daily-room-slot.drag-target')],

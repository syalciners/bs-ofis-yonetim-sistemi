...
export function CollectionQuickForm({ onDone, onCancel }: {onDone:()=>void;onCancel:()=>void}) {
  ...
  return <form className="form-grid" onSubmit={async e=>{e.preventDefault();setBusy(true);const f=new FormData(e.currentTarget);try{await saveCollection({ogrenci_id:studentId,tutar:Number(f.get('tutar')),tarih:String(f.get('tarih')),odeme_yontemi:String(f.get('odeme_yontemi')),aciklama:String(f.get('aciklama')||'')||null});await refresh();toast('Tahsilat ve kasa hareketi kaydedildi.');onDone()}catch(err:any){toast(err.message||String(err),'error')}finally{setBusy(false)}}}>
    <label className="wide">Öğrenci<select value={studentId} onChange={e=>{setStudentId(e.target.value);setAmount('')}} required autoFocus><option value="">Öğrenci seçin</option>{data.ogrenciler.filter(x=>x.durum!=='Pasif').sort((a,b)=>a.ad_soyad.localeCompare(b.ad_soyad,'tr')).map(x=><option key={x.ogrenci_id} value={x.ogrenci_id}>{x.ad_soyad}</option>)}</select></label>
    ...
    <div className="wide form-hint">Borçtan fazla ödeme alınabilir; fazla tutar öğrenci bakiyesinde avans olarak kalır.</div>

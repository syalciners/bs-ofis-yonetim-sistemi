...
const checks=[
  ...
  ['Kurum ayarı tek public-okunur kaynaktan yüklenir',institutionService.includes("from('kurum_ayarlari')")&&institutionService.includes("eq('kurum_id', 'ANA')")&&institutionService.includes('maybeSingle()')],
  ...
  ['Kurum ayarı oturum öncesinde ortak duruma yüklenir',provider.includes('loadInstitutionSettings().then')&&provider.includes('institution: KurumAyarlari | null')],
  ['Normal yenileme kurum ayarını da tekrar yükler',provider.includes('nextInstitution')&&provider.includes('loadInstitutionSettings()')&&provider.includes('setInstitution(nextInstitution)')],

...
export function SystemPage(){
  ...
  return <div className="page-stack">
    <section className="page-title-row"><div><span className="eyebrow">SİSTEM</span><h1>Sistem Durumu</h1><p>Veri bütünlüğü ve program sağlığı.</p></div><button className="secondary-btn" onClick={()=>void refresh()}><RefreshCw size={17} className={refreshing?'spin':''}/>Veriyi Yenile</button></section>
    ...
    {result&&<>
      ...
      <section className="mini-grid two"><div><span>Çekirdek Sistem</span><b className={system?.basarili?'success-text':'danger-text'}>{system?.basarili?'Sağlıklı':`${Number(system?.toplam_hata||0)} sorun`}</b></div><div><span>Program Sistemi</span><b className={program?.basarili?'success-text':'danger-text'}>{program?.basarili?'Sağlıklı':`${Number(program?.toplam_hata||0)} sorun`}</b></div></section>
    ...
    <section className="settings-list"><div><Database/><span><b>Veri Kaynağı</b><small>Supabase PostgreSQL · RLS etkin</small></span></div><div><Server/><span><b>Yazma Mimarisi</b><small>Kritik işlemler güvenli RPC / transaction katmanı üzerinden çalışır.</small></span></div><div><TriangleAlert/><span><b>Güvenlik</b><small>Zoom secret ve servis kimlik bilgileri tarayıcıya taşınmaz.</small></span></div></section>
  </div>
}

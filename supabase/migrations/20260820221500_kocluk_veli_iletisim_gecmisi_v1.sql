...
begin
  ...
  if v_kanal not in ('Kopyalama', 'WhatsApp') then
    raise exception 'Geçersiz veli iletişim kanalı.';
  end if;

  if v_icerik is null then
    raise exception 'İletişim içeriği boş olamaz.';
  end if;
  if char_length(v_icerik) > 4000 then
    raise exception 'İletişim içeriği çok uzun.';
  end if;

  v_durum := case when v_kanal = 'WhatsApp' then 'WhatsApp''ta Açıldı' else 'Kopyalandı' end;

  -- Aynı mesaj aynı kanalda kısa süre içinde tekrar tıklanırsa geçmişi şişirme.
  select v.iletisim_id
    into v_id
  ...
  if v_id is not null then
    v_tekrar := true;
    update public.kocluk_veli_iletisimleri
      set iletisim_zamani = now(), durum = v_durum
    where iletisim_id = v_id;
  else

(function(){
  ...
  function stilEkle(){
    ...
    s.textContent=`
      .bstg-modal{position:fixed;inset:0;z-index:975;display:none;align-items:flex-end;justify-content:center;background:rgba(15,23,42,.46);backdrop-filter:blur(5px);-webkit-backdrop-filter:blur(5px)}
      .bstg-modal.acik{display:flex}
      ...
      .bstg-ust{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:15px}
      .bstg-baslik{font-size:21px;font-weight:850;color:var(--yazi);line-height:1.2}.bstg-alt{margin-top:5px;color:var(--ikincil);font-size:10.5px;line-height:1.45}
      .bstg-kapat{width:40px;height:40px;flex:0 0 auto;border:1px solid var(--kenar);border-radius:12px;background:#fff;color:var(--ikincil);font-size:23px;cursor:pointer}
      .bstg-alan{display:flex;flex-direction:column;gap:6px;margin-bottom:11px}.bstg-alan label{font-size:10px;font-weight:780;color:var(--ikincil)}
      .bstg-alan select,.bstg-alan input{width:100%;height:48px;border:1px solid var(--kenar);border-radius:13px;background:#fff;color:var(--yazi);font:inherit;font-size:14px;outline:none;padding:0 13px}.bstg-alan select:focus,.bstg-alan input:focus{border-color:#93c5fd;box-shadow:0 0 0 3px rgba(37,99,235,.08)}
  ...
  function modalHazirla(){
    ...
    m.innerHTML=`<div class="bstg-sheet"><div class="bstg-ust"><div><div class="bstg-baslik">Tahsilat Gir</div><div class="bstg-alt">Öğrenciyi seçin, tutarı yazın. Ödeme hesabını sistem otomatik belirler.</div></div><button id="bstgKapat" class="bstg-kapat" type="button">×</button></div><form id="bstgForm" autocomplete="off"><div class="bstg-alan"><label for="bstgOgrenci">Öğrenci</label><select id="bstgOgrenci" required><option value="">Öğrenci seçin</option></select></div><div id="bstgBakiye" class="bstg-bakiye"><div><span id="bstgBakiyeEtiket">Güncel bakiye</span><strong id="bstgBakiyeDeger">—</strong></div><button id="bstgKalanSec" class="bstg-kalan-buton" type="button">Kalanı seç</button></div><div class="bstg-alan"><label for="bstgTutar">Tutar</label><input id="bstgTutar" type="number" min="1" step="1" inputmode="decimal" placeholder="0" required></div><div class="bstg-alan"><label>Ödeme Yöntemi</label><div class="bstg-yontemler"><button class="bstg-yontem aktif" type="button" data-bstg-yontem="Havale/EFT">Havale / EFT</button><button class="bstg-yontem" type="button" data-bstg-yontem="Nakit">Nakit</button></div></div><button id="bstgDetayToggle" class="bstg-detay-toggle" type="button">Tarih ve açıklama ekle</button><div id="bstgDetay" class="bstg-detay"><div class="bstg-alan"><label for="bstgTarih">Tarih</label><input id="bstgTarih" type="date" required></div><div class="bstg-alan"><label for="bstgAciklama">Açıklama</label><input id="bstgAciklama" maxlength="300" placeholder="İsteğe bağlı"></div></div><div id="bstgSonuc" class="bstg-sonuc"></div><div class="bstg-altbar"><div class="bstg-not">Bu test sürümünde kayıt yazılmaz; sonraki adımda güvenli tahsilat + kasa işlemi tek işlem olarak bağlanacak.</div><button id="bstgDevam" class="bstg-devam" type="submit">Tahsilatı Hazırla</button></div></form></div>`;
    document.body.appendChild(m);
    m.addEventListener('click',e=>{if(e.target===m)kapat();});
    document.getElementById('bstgKapat').addEventListener('click',kapat);
    document.getElementById('bstgOgrenci').addEventListener('change',ogrenciDegisti);
    document.getElementById('bstgKalanSec').addEventListener('click',kalaniSec);
    document.getElementById('bstgTutar').addEventListener('input',sonucuTemizle);
    document.getElementById('bstgTarih').addEventListener('change',sonucuTemizle);
    document.getElementById('bstgAciklama').addEventListener('input',sonucuTemizle);
    ...
    document.getElementById('bstgForm').addEventListener('submit',hazirla);
  }

  async function verileriYukle(){
    if(ref&&bakiyeler.size) return;
    if(!window.BSReferansServisi||!window.BSFinansServisi) throw new Error('Finans servisleri yüklenmedi.');
    const [r,b]=await Promise.all([BSReferansServisi.yukle(),BSFinansServisi.borcOzetiGetir()]);
    ref=r;
    bakiyeler=new Map((b.bakiyeler||[]).map(x=>[x.ogrenci_id,Number(x.kalan)||0]));
    const s=document.getElementById('bstgOgrenci');
    s.innerHTML='<option value="">Öğrenci seçin</option>'+r.ogrenciler.filter(x=>x.durum!=='Pasif').map(x=>`<option value="${htmlKacir(x.ogrenci_id)}">${htmlKacir(x.ad_soyad||'Öğrenci')}</option>`).join('');
  }
  ...
  function bakiyeGoster(id){
    const kutu=document.getElementById('bstgBakiye'),etiket=document.getElementById('bstgBakiyeEtiket'),deger=document.getElementById('bstgBakiyeDeger'),buton=document.getElementById('bstgKalanSec');
    if(!id){kutu.classList.remove('goster');return;}
    const kalan=bakiyeler.get(id)||0;kutu.classList.add('goster');
    if(kalan>0.009){etiket.textContent='Açık bakiye';deger.textContent=paraYaz(kalan);buton.style.display='block';}
    else if(kalan<-0.009){etiket.textContent='Ön ödeme bakiyesi';deger.textContent=paraYaz(Math.abs(kalan));buton.style.display='none';}
    else{etiket.textContent='Güncel bakiye';deger.textContent='Borç yok';buton.style.display='none';}
  }
  ...
  async function ac(){
    const m=document.getElementById('bsTahsilatGirModal');m.classList.add('acik');
    document.getElementById('bstgOgrenci').value='';document.getElementById('bstgTutar').value='';document.getElementById('bstgTarih').value=istanbulBugunISO();document.getElementById('bstgAciklama').value='';document.getElementById('bstgDetay').classList.remove('acik');document.getElementById('bstgDetayToggle').textContent='Tarih ve açıklama ekle';document.getElementById('bstgBakiye').classList.remove('goster');sonucuTemizle();
    yontem='Havale/EFT';m.querySelectorAll('[data-bstg-yontem]').forEach(x=>x.classList.toggle('aktif',x.dataset.bstgYontem===yontem));
    const b=document.getElementById('bstgDevam');b.disabled=true;b.textContent='Yükleniyor…';
    try{await verileriYukle();}
    catch(err){console.error('Tahsilat giriş verileri:',err);const s=document.getElementById('bstgSonuc');s.className='bstg-sonuc hata';s.innerHTML='<strong>Bilgiler yüklenemedi</strong>Bağlantı veya erişim yetkisi kontrol edilmeli.';}
    finally{b.disabled=false;b.textContent='Tahsilatı Hazırla';}
  }

  function hazirla(e){
    e.preventDefault();const f=document.getElementById('bstgForm');if(!f.reportValidity()) return;
    const id=document.getElementById('bstgOgrenci').value,tutar=Number(document.getElementById('bstgTutar').value),tarih=document.getElementById('bstgTarih').value;
    const s=document.getElementById('bstgSonuc');
    if(!(tutar>0)){s.className='bstg-sonuc hata';s.innerHTML='<strong>Tutar geçersiz</strong>Tahsilat tutarı sıfırdan büyük olmalı.';return;}
    const ad=ref.ogrenciMap.get(id)||'Öğrenci';
    s.className='bstg-sonuc hazir';s.innerHTML=`<strong>Tahsilat hazır</strong>${htmlKacir(ad)} • ${htmlKacir(paraYaz(tutar))} • ${htmlKacir(yontem)} • ${htmlKacir(tarihKisa(tarih))}`;
  }

  function hedefMi(el){if(!el) return false;const t=(el.textContent||'').trim();return /Tahsilat\s*Gir/i.test(t);}
